...
void* matrixVectorMultiplication(void* arg) {
  ThreadData* data = (ThreadData*)arg;

  for (int i = data->start_row; i < data->end_row; ++i) {
    double* a = data->A + (i * data->N + data->displ_send);

    int j = 0;
    for (; j < data->count_send - 7; j += 8) {
      data->z[i] += data->p_send[j] * a[j] +
                    data->p_send[j + 1] * a[j + 1] +
                    data->p_send[j + 2] * a[j + 2] +
                    data->p_send[j + 3] * a[j + 3] +
                    data->p_send[j + 4] * a[j + 4] +
                    data->p_send[j + 5] * a[j + 5] +
                    data->p_send[j + 6] * a[j + 6] +
                    data->p_send[j + 7] * a[j + 7];
    }

    for (; j < data->count_send; ++j) {
      data->z[i] += data->p_send[j] * a[j];
    }
  }
...
void solve_conjugate_gradient_par(process_data row, equation_data equation, int max_steps, double tol) {
  ...
  while (n < max_steps) {
    ...
    for (int t = 0; t < threads; ++t) {
      thread_data[t].start_row = t * row.count / threads;
      thread_data[t].end_row = (t + 1) * row.count / threads;
      thread_data[t].count = row.count;
      thread_data[t].N = row.N;
      thread_data[t].displ_send = displ_send;
      thread_data[t].count_send = count_send;
      thread_data[t].p_send = p_send;
      thread_data[t].z = z;
      thread_data[t].A = equation.A;

      pthread_create(&thread_handles[t], NULL, matrixVectorMultiplication, (void*)&thread_data[t]);
    }
