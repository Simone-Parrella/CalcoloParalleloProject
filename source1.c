...
void solver(int n, int m) {
  ...
  while (!done && (cnt_iter < MAX_ITER)) {
    ...
    if(myrank % 2 == 0) { // I ranks pari inviano e poi ricevono
      if(myrank != np-1) {
        MPI_Isend(&vector[(n-2)*m], m, MPI_FLOAT, myrank+1, 1, MPI_COMM_WORLD, &request); // invia penultima
        MPI_Recv(&vector[(n-1)*m], m, MPI_FLOAT, myrank+1, 1, MPI_COMM_WORLD, &status); // ricevi in ultima
        MPI_Wait(&request, &status); // attendi la fine dell'invio
      }

      if(myrank != 0) {
        MPI_Isend(&vector[m], m, MPI_FLOAT, myrank-1, 1, MPI_COMM_WORLD, &request); // invia seconda
        MPI_Recv(&vector[0], m, MPI_FLOAT, myrank-1, 1, MPI_COMM_WORLD, &status); // ricevi in prima
        MPI_Wait(&request, &status); // attendi la fine dell'invio
      }
    }
